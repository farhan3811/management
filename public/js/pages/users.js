$(function () {
    ...
    $('.js-exportable-ajax, .header').on('click', '.form-users', function () {
        ...
        $.ajax({
            url: $('.mainurl').val() +'/users/create/'+cd+'/'+this.id.substr(0,2),
            type: "GET",
            cache: false,
    ...
    /* -------------------------------- SAVE  ------------------------------- */
    $('#largeModal').on('click', '#save-users', function (e) {

        if($('#username_data').val().length < 6 || $('#password_data').val().length < 6 )
        {
            alert('Username / Password kurang dari 6 huruf');
        }else if($('#roles').val().length < 1 ){
            alert('Level User harus diisi');
        }else if($('#nik_data').val().length < 1 ){
            alert('NIK harus diisi');
        }else if($('#nama_data').val().length < 1 ){
            alert('Nama harus diisi');
        }else{

            $.ajax({
                url: $('.mainurl').val() +'/users/save',
                type: "POST",
                data:{
                    ...
                    nik                 : $('#nik_data').val(),
                    nama                : $('#nama_data').val(),
                    gender              : $('#gender_data').val(),
                    telepon             : $('#telepon_data').val(),
                    handphone           : $('#handphone_data').val(),
                },
                cache: false,
                success:function(data)
                {
